...
func validateDataPoints(query influxql.Statement, options *Options) error {
	...
	if stmt, ok := query.(*influxql.SelectStatement); ok {
		...
		buckets, err := getNumberOfBuckets(stmt, now, &nowValuer)
		if err != nil {
			return err
		}
...
func getNumberOfBuckets(stmt *influxql.SelectStatement, now time.Time, nowValuer *influxql.NowValuer) (int, error) {
	...
	if interval > 0 {
		...
		min := timeRange.Min.Truncate(interval)
		max := timeRange.Max.Truncate(interval).Add(interval)
